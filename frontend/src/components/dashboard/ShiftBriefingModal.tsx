...
export default function ShiftBriefingModal({
	...
	return (
		<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
			<div
				className={`bg-white w-full rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 transition-all ${generatedText ? "max-w-4xl" : "max-w-lg"}`}
			>
					...
					{!generatedText ? (
						<div className="flex flex-col gap-1.5">
							...
							<p className="text-xs text-slate-500 mb-1">
								Enter a value between 1 and 24 to summarize recent activity.
							</p>
							<input
								type="number"
								min={1}
						...
						<div className="relative group">
							...
							<div className="w-full h-[60vh] min-h-[400px] p-6 bg-slate-50 border border-slate-200 rounded-md overflow-y-auto prose prose-sm prose-slate max-w-none prose-headings:text-[#1a237e] prose-strong:text-slate-800">
				...
				<div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
					...
					{!generatedText && (
						<button
							...
							disabled={isGenerating || !hours || hours < 1 || hours > 24}
							className="flex items-center justify-center min-w-[120px] px-4 py-2 text-sm font-semibold text-white bg-[#1a237e] rounded-md hover:bg-[#121858] transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
						>
							{isGenerating ? (
